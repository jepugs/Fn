...
The search path takes its value from the environment variable ~FN_SEARCH_PATH~
if set. Otherwise, it defaults to
~.:${HOME}/.local/lib/Fn/ns:${PREFIX}/lib/Fn/ns~. Here ~${HOME}~ is the user's
home directory (environment variable) and ~${PREFIX}~ is a directory determined
at build time, but probably ~/usr~.

Syntax here is like for the ~PATH~ environment variable. Backslash can be used
as an escape character if one of the paths includes a colon.

Note that ~.~ is the first directory in the path, so it is recommended that .fn
files go in dedicated project directories in order to prevent name collisions.
