...
public:
    ...
    SourceLoc* locationOf(Addr addr) const;
    ...
    optional<Value> findSymbol(const string& name) const;
    ...
    inline const u8& operator[](Addr addr) const {
        return data[addr];
    }
...
private:
    ...
    Value peek(StackAddr offset=0) const;
    // get a local Value from the current call frame
    Value local(Local l) const;
    // set a local value
...
public:
    ...
    UpvalueSlot getUpvalue(Local id) const;
    ...
    void runtimeError(const string& msg) const;
