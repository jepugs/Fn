...
constant_id add_const(function_tree* dest, value v);
constant_id add_number_const(function_tree* dest, f64 number);
constant_id add_string_const(function_tree* dest, const string& str);
constant_id add_quoted_const(function_tree* dest, ast_form* to_quote);
// add a subfunction to a function tree. This causes a new stub to be created
// too.
function_tree* add_sub_fun(function_tree* dest);
// these are for writing code to the function tree. FIXME: should move these to
// the compiler.
constant_id writeu8(function_tree* dest, u8 u);
...
public:
    // Fill out the function tree. This sets the llir_forms in the tree,
    // creating subtrees as necessary.
    void expand(function_tree* dest, ast_form* form);
};
