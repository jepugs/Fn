...
- [[Language Guide]] contains a walkthrough of all the major features of the Fn
  language. It's meant to be something you can read in order.

- [[Command Line Interface]] gives instructions on using the ~fn~ command line
  tool to evaluate code.

- [[Builtin Functions]] contains a list of builtin functions. This section is
  mostly unwritten.
...
To get started, let's take a look at the basic syntax. Fn uses *parenthesized
...
(def x 42)        ; => defines a global variable
...
It's only crucial to read [[Checking Types]] and the opening paragraphs of [[Lists]] and
[[Tables]].
